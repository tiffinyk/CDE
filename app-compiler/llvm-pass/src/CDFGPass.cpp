...
std::vector<std::map<int, Loop*>> getMappingUnitNameUsingLoopMark(Function &F, std::map<std::string, MappingUnit> &mappingUnitMap, std::map<const llvm::BasicBlock *, std::set<const llvm::BasicBlock *>> SuccBBsMap)
{
	...
	if(LoopStart == NULL && LoopEnd == NULL){
		...
		for (auto &elem : mappingUnitMap){
			...
			auto loopBBs = nestLoop[i-1]->getBlocks();
			for(auto BB : loopBBs){
...
/////////////////////////////////////////
/// Folloing function is added by Mion Lou in 20230424
////////////////////////////////////////
// std::vector<std::map<int, Loop*>> getMappingUnitNameOfSingleLoop(std::map<std::string, MappingUnit> &mappingUnitMap)
// {
//   LoopInfo &LI = getAnalysis<LoopInfoWrapperPass>().getLoopInfo();
  
//   for (Loop *L : LI) {
// 		llvm::errs() << "[debug] loop: \n";
// 		L->dump();
//     BasicBlock *LoopHeader = L->getHeader();
//     BasicBlock *LoopLatch = L->getLoopLatch();
//     for (BasicBlock *BB : L->getBlocks()) {
//       if (BB != LoopHeader && BB != LoopLatch) {
//         // This basic block is inside the loop
...
/////////////////////////////////////////
/// Above function is added by Mion Lou in 20230424
////////////////////////////////////////
...
namespace {
    cl::opt<std::string> targetFuncName("fn", cl::init("na"), cl::desc("function name"));
		cl::opt<bool> MappingAll("mapping-all", cl::init(false), cl::desc("The whole function is a complete kernel (default to be false)"));
    ...
    struct CDFGPass : public FunctionPass {
      ...
      std::vector<std::map<int, Loop*>> LoopVec = getMappingUnitNameUsingLoopMark(F, MappingUnitMap, SuccBBsMap);//check LoopMark first
			
			/////////////////
			////// Following "if" is Added by Mion Lou in 20230424
			/////////////////
			if(MappingAll == true){
				Loop *tem = MappingUnitMap.begin()->second.lp;
				int i = 0;
				nestloops.clear();
				while (tem != nullptr)
				{
				...
				LLVMCDFG *CDFG = new LLVMCDFG(targetFuncName);
				CDFG->SE = SE;
				CDFG->DT = DT;
				CDFG->DI = DI;
				CDFG->DL = &DL;
			...
			/////////////////
			////// Above is Added by Mion Lou in 20230424
			/////////////////
