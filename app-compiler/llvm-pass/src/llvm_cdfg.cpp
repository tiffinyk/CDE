...
void LLVMCDFG::connectCtrlDepBBs(){
	...
	for(auto &elem : CtrlDepPredBBs){
		...
		for(auto &ctrlDepPredBB : elem.second){
            ...
            //to AND each CTRL path's node to tem preBB
			...
			for(LLVMCDFGNode* succNode : ctrlDepNodes){
                ...
                succNode->addInputNode(preBRNode, idx, isBackEdge, cond); // donot care the index
                addEdge(preBRNode, succNode, EDGE_TYPE_CTRL);
			}
	...
	for(auto &elem : temnodes){
		...
		if(node->instruction() && dyn_cast<PHINode>(node->instruction()) && node->customInstruction() != "SELECT"){
            ...
            for (int i = 0; i < PHI->getNumIncomingValues(); ++i)
            {
                ...
                // errs() << "[debug] bb:\n"; bb->dump();
                // errs() << "[debug] BRNode getName:" << BRNode->getName() <<"\n";
                if (BRI->isConditional())
                {                                             //
    ...
    while(flag){ // scan interatively
        ...
        for (auto &elem : nodes)
        {
            ...
            if (customIns == "CTRLAND" || customIns == "CTRLOR")
            {
                ...
                {
                    if((isNotOp0 && op0->outputNodes().size() > 1) | (isNotOp1 && op1->outputNodes().size() > 1) | (isNotDst && node->outputNodes().size() > 1)){
                        continue;
                    }
    ...
    while(removed){
        ...
        for(auto &elem : _nodes){
            ...
            if(node->inputNodes().empty()){ // no input node
                if(customIns != "CONST" && customIns != "LOOPSTART" && customIns != "INPUT" && customIns != "ArgIn" && !node->isLSaffine()){
    ...
    for(auto &elem : _nodes){
        ...
        if(!customIns.empty()){
            ...
            case Instruction::FAdd:
                // node->setFinalInstruction("FADD"); fix by ljh in 20230619
                node->setFinalInstruction("FADD32");
                break;
            case Instruction::Sub:
                node->setFinalInstruction("SUB");
                break;
            case Instruction::FSub:
                // node->setFinalInstruction("FSUB"); fix by ljh in 20230619
                node->setFinalInstruction("FSUB32");
                break;
            case Instruction::Mul:
                node->setFinalInstruction("MUL");
                break;
            case Instruction::FMul:
                // node->setFinalInstruction("FMUL"); fix by ljh in 20230619
                node->setFinalInstruction("FMUL32");
                break;
            ...
            case Instruction::FDiv:
                node->setFinalInstruction("FDIV32");
                break;
...
//get every nests' idx stride & bound
    for(int level = 0; level < total_level; level++){
        ...
        //////
        // Fixed by Mion Lou 20230425 
        //  to apply to the situation that the icmp of branch is in header BB instead of Latch BB
        //////
        // errs() << "[debug] loop level: " << level << "\n";
        // errs() << "[debug] idxPhi: " << *idxPhi << "\n";
        // nestloops[level]->dump();
        BranchInst *BI_Latch = dyn_cast_or_null<BranchInst>(nestloops[level]->getLoopLatch()->getTerminator());
        // BI_Latch->dump();
        Loop::LoopBounds* LoopBound;
        Instruction *idx_gen_oprand;
        Value *InitialIVValue, *FinalIVValue ;
        bool IsAffine = false;
        if(BI_Latch->isUnconditional()){
            /// The terminator of the latch block is unconditional
            /// which means the icmp of branch is in header BB
            // nestloops[level]->getHeader()->dump();
            // LoopBound = Loop::LoopBounds::getBounds(*(nestloops[level]) ,*idxPhi, *SE).getPointer();

            //////////// Loop::LoopBounds::getBounds
            llvm::Loop* L = nestloops[level];
            llvm::InductionDescriptor IndDesc;
            if (!InductionDescriptor::isInductionPHI(idxPhi, L, SE, IndDesc)){
                assert(1);
            }

            InitialIVValue = IndDesc.getStartValue();
            ...
            if (Op0 == idxPhi || Op0 == StepInst){
                FinalIVValue = Op1;
                idx_gen_oprand = StepInst;
            }

            else if (Op1 == idxPhi || Op1 == StepInst){
                FinalIVValue = Op0;
                idx_gen_oprand = StepInst;
            }
            /// Following 3 variables is got:
            ///  Instruction *idx_gen_oprand; Value *InitialIVValue, *FinalIVValue ;
        
            // LoopBound = Loop::LoopBounds(*L, *InitialIVValue, *StepInst, StepValue, *FinalIVValue, *SE);
            // errs() << "[debug] FinalIVValue: " << *FinalIVValue << "\n";
            // errs() << "[debug] idx_gen_oprand: " << *idx_gen_oprand << "\n";
            // errs() << "[debug] InitialIVValue: " << *InitialIVValue << "\n";
            IsAffine = true;
        }

        else if (BI_Latch->isConditional())
        {   
            LoopBound = nestloops[level]->getBounds(*SE).getPointer();
            idx_gen_oprand = &(LoopBound->getStepInst());
            // Value* InitialIVValue, *FinalIVValue; /// Declared outside this "if"
        ...
        if(IsAffine){
            ...
            if(exitcmp->getOperand(0) == idxPhi){
                if(bounds[level].second.index() == 0){
                    ...
                    bounds[level].second = bounds[level].second; 
                }else{
...
std::vector<PHINode*> LLVMCDFG::arrayStride(LLVMCDFGNode* opnode, std::map<int, std::pair<double,std::pair<varType, varType>>>* factortable){
    ...
    llvm::errs() << "[debug] Input port node: "<<opnode->getName() << "\n";
    //if return {} && factortable->clear() means non-affine operation
    if(factortable == NULL || factortable->empty()){
        if(factortable != NULL){factortable->clear();}
        return {};
        ...
        else
        {
            for (int i = 0; i < factortable.size(); i++)
            {
                ...
                errs() << "\t\tin this GEP " << i 
                       ...
                       << " ; leftbound: " << factortable[i].second.first.to_string() 
                       << " ; rightbound: " << factortable[i].second.second.to_string() 
                       << "\n";
            }
        }
...
void LLVMCDFG::handleSelectNodes(){
    ...
    for (auto &elem : nodes)//Condition(all LS affine)'s SELECT delete has been dealed integrated below
    {
        ...
        if (node->customInstruction() == "SELECT")
        {
            ...
            if (pre->isOutputBackEdge(node) && hasinitial)
            {
                ...
                // assert(initSels.size() > 0);
                // if (initSels.size() > 1)
                // {
                //     for (auto selout : initSel->outputNodes())
                //     {
                //         selout->addInputNode(node, selout->getInputIdx(initSel), selout->isInputBackEdge(initSel));
                //         node->addOutputNode(selout, initSel->isOutputBackEdge(selout));
                //         addEdge(node, selout, edge(initSel, selout)->type());
                //     }
                //     for (auto elem : initSels)
                //     {
                //         if (elem.first != node)
                //             rmnodes.insert(elem.first); // finally delete these SELECT
                //     }
                // }
    ...
    {
        auto node = elem.second;
        if (node->customInstruction() == "InitSELECT"){
            ...
            for (auto selectout : temoutputs)
            {
                ...
                if (selectout->instruction() != NULL && selectout->instruction()->getOpcode() != Instruction::Add)
            ...
            if (findCACC)
            {
                ...
                /// TODO:copy add node when both SELECT and add have outputnodes
                bool CACCfirst = true;
...
void LLVMCDFG::generateCDFG()
{
    ...
    // initialized CDFG
    unsigned dot_cnt = 0;
    printDOT(std::to_string(dot_cnt++) +"_"+_name + "_init.dot");
    ...
    addMemDepEdges();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_addMemDepEdges.dot");

    // Connect control dependent node pairs among BBs
    outs() << ">>>>>> Connect control dependent node pairs among BBs\n";
    connectCtrlDepBBs();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_connectCtrlDepBBs.dot");
    ...
    insertCtrlNotNodes();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_insertCtrlNotNodes.dot");
    // transfer the multiple control predecessors (input nodes) into a inverted OR tree 
    // with the root connected to a node and leaves connected to control predecessors
    outs() << ">>>>>> Transfer multiple control predecessors (input nodes) into a inverted OR tree\n";
    createCtrlOrTree();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_createCtrlOrTree.dot");

    outs() << ">>>>>> Transfer GEP node to MUL/ADD/Const tree\n";
    handleGEPNodes();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_handleGEPNodes.dot");

    affineAnalyze();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_affineAnalyze.dot");

    outs() << ">>>>>> Transfer PHINode to SELECT nodes\n";
    handlePHINodes(); 
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_handlePHINodes.dot");

    outs() << ">>>>>> Handle select Node\n";
    handleSelectNodes(); 
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_handleSelectNodes.dot");

    outs() << ">>>>>> affine Access Node\n";
    handleAffineLSNodes(); 
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_handleAffineLSNodes.dot");
    ...
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_handleConversionNodes.dot");

    outs() << ">>>>>> fuse constant input node with the mul output node for acc node\n";
    fuseAccConstNode();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_fuseAccConstNode.dot");

    outs() << ">>>>>> simplify the control logic\n";
    simplifyCtrlLogic();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_simplifyCtrlLogic.dot");

    outs() << ">>>>>> Remove redundant nodes\n";
    removeRedundantNodes();
    printDOT(std::to_string(dot_cnt++) +"_"+ _name + "_after_removeRedundantNodes.dot");
