...
void ADGNode::delOperandInputs(int opeIdx, int inputIdx){
    assert(_operandInputs.size() > opeIdx);
    if(inputs().count(inputIdx)){
...
void ADGNode::addOperandInputs(int opeIdx, std::set<int> inputIdxs){
    assert(_operandInputs.size() > opeIdx);
...
int ADGNode::getOperandIdx(int inputIdx){
...
bool ADGNode::outReged(int idx){
    return _outReged[idx];
}

// if there are registers in the output port 
void ADGNode::setOutReged(int idx, bool reged){
    _outReged[idx] = reged;
}