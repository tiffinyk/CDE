 ...
 */
class CGRA(attrs: mutable.Map[String, Any], getArea: Boolean, dumpIR: Boolean, outFilename: String = "") extends Module with IR{
  // CGRA parameters
  ...
  val sm_id_attrs = mutable.Map[Int, Map[String, Any]]()
  // sub-module instance id to attribute
  val smi_id_attrs = mutable.Map[Int, Map[String, Any]]()
...
//      area = area + ppa.ppa_iob.getiobarea(1,numOutIB)
      val smi_id_attr: Map[String, Any] = Map(
...
//      area = area +  ppa.ppa_gib.getgibarea(numTrack,GibParam.diag_iopin_connect,GibParam.num_iopin_list,GibParam.connect_flexibility,GibParam.track_reged,GibParam.trackDirections)
  ...
  apply("sub_modules", sub_modules.toList)

  val instances = smi_id.map{case (name, ids) =>
    ids.map{id => Map(
      "id" -> id, 
      "type" -> name) ++ 
      {if(name != "This") smi_id_attrs(id) else Map[String, Any]()}
    }
  }.flatten
  apply("instances", instances.toList)
...
//    connections.append((smi_id("OB")(i), "OB", 0, smi_id("This")(0), "This", i))
  ...
  apply("connections", connections.zipWithIndex.map{case (c, i) => i -> c.productIterator.toList}.toMap)
  ...
  if(getArea){
    val area = estimator.get_area(attributes.toMap)
